...
# Confidence - Prediction of the Kircher et alâ€™s confidence score which is a re-scale of the p-value fit to the experimental data. A â€˜significant expression effectâ€™ in the provided data is one assigned a confidence score greater than or equal to 0.1.
...
my $filename = $ARGV[0];
if ($filename =~ /^([A-Za-z\-\_\.\/0-9]+)$/){
#if ($filename =~ /^([A-Za-z\-\_\.]+)$/){
    $filename = $1;
}
else {
    die "Only alphabet, \"_\", \"-\" and \".\" are allowed in the file name\n";
}
...
for(my $line=1;$line<=@contents;$line++){
    ...
    push @errors,"The number of columns at line $line should be exact $num_columns. No empty cells allowed, except for comments.\n" if @cols < $num_columns;
	...
	if($cols[5] != -1 && $cols[5] != 0 && $cols[5] != 1){
	    push @errors, "The 6th column (Direction) at line $line should be 1, 0 or -1; No empty cells allowed..\n"
	} else {}
  if($cols[6]=~/^[+-]?\d+[.]?\d*$/){
      push @errors, "The 7th column (P Direction) at line $line should be a real number between 0 and 1; No empty cells allowed..\n" if($cols[6]>1 || $cols[6]<0);
  }
  else{
      push @errors, "The 7th column at line $line should be a real number between 0 and 1; No empty cells allowed..\n"
  }
  if($cols[7]=~/^[+-]?\d+[.]?\d*$/){
	    push @errors, "The 8th column (confidence) at line $line should be a real number between 0 and 1; No empty cells allowed..\n" if($cols[7]>1 || $cols[7]<0);
	}
	else{
	    push @errors, "The 8th column at line $line should be a real number between 0 and 1; No empty cells allowed..\n"
	}
	if($cols[8]=~/^[+-]?\d+[.]?\d*$/){
	    push @errors, "The 9th column at line $line should be a real number between 0 and 1; No empty cells allowed..\n" if($cols[8]>1 || $cols[8]<0);
	}
	else{
	    push @errors, "The 9th column at line $line should be a real number between 0 and 1; No empty cells allowed..\n"
	}
