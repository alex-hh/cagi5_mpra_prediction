...
plot_element <- function(.df) {
  ...
  ggplot(.df, aes(x = Pos, y = Value, colour = Confidence)) +
    ...
    scale_colour_gradient(limits = c(0, 1), low = muted('blue'), high = muted('red'))
...
ggplot(
  ...
  geom_boxplot() +
  facet_wrap(~ elem)
ggsave('../plots/spatial.pdf', width = 16, height = 12, units = 'in')
```
...
for (e in unique(training_df$elem)) {
  ...
  gp <- ggplot(
      training_df %>%
        ...
        mutate(subst = paste(Ref, Alt, sep = '')),
      aes(x = subst, y = Value)) +
    geom_boxplot(alpha = .3, outlier.alpha = 0) +
